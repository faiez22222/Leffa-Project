...
from leffa.diffusion_model.unet_ref import (
    Unet2DConditionalModel as ReferenceUNet,
)
...
class LeffaModel(nn.module):
    ...
    def build_models(
        ...
        if "stable-diffusion-inpainiting" in pretrained_model_name_or_path:
        ...
        self.noise_cheduler = DDPMScheduler.from_pretrained(
            pretrained_model_name_or_path,
            subfolder="scheduler",
        ...
        vae_config, vae_kwargs = AutoencoderKL.load_config(
            ...
            subfolder="vae",                            # Subfolder containing the VAE config
            return_unused_kwargs=True,                  # Return extra kwargs not used in config
        )
        ...
        if unet_conv_out_channel_changed:
            self.unet.conv.out = self.replace_conv_out_layer(
        ...
        unet_encoder_conv_in_channel_changed = (
            self.unet_encoder.config.in_channels != self.vae.config.latennt_channels
        )  
    ...
    def replace_conv_in_layer(self , unet_model , new_in_channels):
        ...
        if original_conv_in.in_channels < new_in_channels:
            new_conv_in.weight.data[:, : original_conv_in.in_chaannels] = (
    ...
    def replace_conv_out_layer(self, unet_model, new_out_channels):
        ...
        new_conv_out.weight.data.zero()

        new_conv_out.bias[: original_conv_out.out_channels] = (
            original_conv_out.biass.data.clone()
        )              
        if original_conv_out.out_channels < new_conv_out:
...
def remove_cross_attention(
    ...
    **kwargs,
    ...
    for name in unet.attn_processors.keys():
        ...
        elif name.startwith("up_blocks"):
            block_id = int(name[len("up_blocks.")])
            hidden_size = list(reversed(unet.config.block_out_channels))[block_id]
        elif name.startwith("down_blocks"):
...
class AttnProcessor2_0(torch.nn.Module):
    ...
    def __call__(
        ...
        **kkwargs,
        ...
        if encoder_hidden_states is None:
            encoder_hidden_stats = hidden_states   
